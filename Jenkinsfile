pipeline {
    ...
    stages {
        stage('Build') {
            ...
            steps {
                sh "fpm -a amd64 --python-bin python3 --python-pip pip3 --python-package-name-prefix python3 -n passerelle-imio-sso-agents -s python -t deb -v `echo ${VERSION}` --python-install-lib /usr/lib/python3/dist-packages -d passerelle setup.py"
        ...
        stage('Push deb to buster apt repo') {
            ...
            steps {
                withCredentials([usernameColonPassword(credentialsId: 'nexus-teleservices', variable: 'CREDENTIALS'),string(credentialsId: 'nexus-url-buster', variable:'NEXUS_URL_BUSTER')]) {
                    sh ('curl -v --fail -u $CREDENTIALS -X POST -H Content-Type:multipart/form-data --data-binary @passerelle-imio-sso-agents_`echo ${VERSION}`_amd64.deb $NEXUS_URL_BUSTER')
                }
            }
        }
